...
function generateIssueMarkdown(issue) {
  ...
  fs.writeFile(`./docs/timeline/issue-${issue.number}.md`, content, 'utf8').then(() =>
...
`
  ...
  fs.writeFile('./docs/timeline/index.md', content, 'utf8').then(() =>
  ...
  const jsonContent = issueYearGroups.map(([year, issues]) => ({
    ...
    items: issues.map((issue) => ({
      text: issue.title,
      link: `/timeline/issue-${issue.number}`
    }))
  }))

  fs.writeJSON('./scripts/timeline.json', jsonContent, {
    spaces: 2,
    encoding: 'utf8'
